...
--
-- Order matters: create every table FIRST, then the functions, then all the
-- RLS policies (policies reference other tables, so those tables must exist).
--
-- Model:
...
--   invoices        — invoices a client pays via Stripe Checkout.
--
-- RLS:
...
-- the signed-in user's email, lowercased (for email-based client access)
...
-- app_staff: a user may read only their own row. No write policy on purpose —
-- staff are seeded from the SQL editor / service role (you can't self-promote).
